...
- 실행 시 트레이 아이콘이 생성됩니다.
- 트레이 메뉴를 통해 **매핑 On/Off, 설정, 업데이트, 종료** 기능 사용 가능

### 2. 키 매핑 설정

- 트레이 메뉴의 **설정**을 열어 프리셋·키 매핑·매크로를 GUI로 편집합니다.
- 색상/좌표/드래그 값은 **화면에서 집기** 버튼으로 게임 화면을 클릭해 자동 입력할 수 있어, `config.json`을 직접 편집할 필요가 없습니다.
- 키 프리셋은 여러 세트를 저장할 수 있으며, 전환 키를 통해 손쉽게 변경할 수 있습니다.
- 설정은 `config.json`에 저장되며, 직접 편집도 그대로 지원됩니다.
...
Rust(MSVC 툴체인)만 있으면 됩니다.

```bash
cargo build --release
```

결과물은 `target/release/umakey.exe` **단일 실행 파일**입니다.  
아이콘, 관리자 권한 매니페스트, 설정 편집기, 업데이터가 모두 이 exe 하나에 포함되며, 실행 시 관리자 권한으로 자동 승격됩니다.
